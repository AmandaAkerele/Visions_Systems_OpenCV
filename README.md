...
# Main function for executing your automated logic - add other transformation functions above as needed
...
def execute_pipeline(environment: str,
    ...
    for index in range(0, 5):
        ...
        joined_other = full_other.join(
            amcare_filtered.withColumnRenamed("facility_name", "facility_name_org").withColumnRenamed("org_id","org_id_org")
            .filter(col('submission_period') < 13),
            ["facility_am_care_num", "submission_period"], "left_outer"
        )
        ...
        zero_submission_counts_qc_bc = joined_qc_bc.filter(col("total_count").isNull()) \
            ...
            .agg(F.count("*").alias("Periods_with_zero_counts"))
        ...
        ed_nodup_nosb_22 = ed_nodup_nosb_22.withColumn('corp_pll_name_e_desc', F.when(ed_nodup_nosb_22.corp_id.isNull(), ed_nodup_nosb_22.org_id).otherwise(ed_nodup_nosb_22.corp_pll_name_e_desc))

        # List of facilities with UCC records
        ...
        tpia_supp_corp_combined = functools.reduce(lambda x, y: x.unionAll(y), tpia_supp_org_all)
        ...
        tpia_peer_rec_df = tpia_peer_cnt_df.groupBy('SUBMISSION_FISCAL_YEAR', 'acute
        ...
        tpia_supp_peer_combined = functools.reduce(lambda x, y: x.unionAll(y), tpia_supp_peer_all)
        ...
        # Remove rows where 'adm_region_id' is null
        ...
        # Stack all dataframes using reduce and unionAll()
        tpia_supp_reg_combined = functools.reduce(lambda x, y: x.unionAll(y), tpia_supp_reg_all)
        ...
        tpia_nt_22 = TPIA_nt_record_ucc_22.groupBy('SUBMISSION_FISCAL_YEAR').agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        
        # Provincial 90th percentile TPIA
        tpia_prov_22 = TPIA_nt_record_ucc_22.groupBy('SUBMISSION_FISCAL_YEAR', 'adm_prov_id', 'adm_prov_plldj_name_e_desc').agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        tpia_prov_22 = tpia_prov_22.filter(tpia_prov_22['adm_prov_id'].isNotNull())

        # Peer Group 90th percentile TPIA
        tpia_peer_22 = ed_record_22_Peer.groupBy('SUBMISSION_FISCAL_YEAR', 'acute_peer_group_code').agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        tpia_peer_22 = tpia_peer_22.filter(tpia_peer_22['acute_peer_group_code'].isNotNull())

        # Regional 90th percentile TPIA
        tpia_reg_22 = ed_record_with_ucc_22.groupBy('SUBMISSION_FISCAL_YEAR', 'adm_prov_plldj_name_e_desc', 'adm_region_id', 'adm_region_pll_name_e_desc').agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        tpia_reg_22 = tpia_reg_22.filter(tpia_reg_22['adm_region_id'].isNotNull())

        # Calculate for SITE_ID/NAME/PEER
        tpia_site_22 = ed_record.groupBy('SUBMISSION_FISCAL_YEAR', 'FACILITY_AM_CARE_NUM', 'acute_peer_group_code').agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        tpia_site_22 = tpia_site_22.filter(tpia_site_22['FACILITY_AM_CARE_NUM'].isNotNull())

        tpia_corp = ed_record.groupBy('SUBMISSION_FISCAL_YEAR', 'CORP_ID').agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        ...
        tpia_org_combined = functools.reduce(lambda x, y: x.unionAll(y), tpia_org_all)
        ...
        final_set = functools.reduce(lambda x, y: x.unionByName(y), stacked_all)

        # Reformat and clean columns
        final_set_df = (final_set.withColumn("reporting_entity_code_text", col('ORG_ID').cast("string"))
    ...
    final_set_df = (final_set_df.withColumn("reporting_period_code", F.concat(F.lit("FY"), F.col("SUBMISSION_FISCAL_YEAR")))
    ...
    tpia_reg_percentiles = tpia_reg_ta_final.groupBy("SUBMISSION_FISCAL_YEAR").agg(
        F.percentile_approx("PERCENTILE_90", 0.2).alias("PERCENTILE_20"),
        F.percentile_approx("PERCENTILE_90", 0.8).alias("PERCENTILE_
    ...
    def process_trend_data(trend_df, id_col):
        ...
        trend_final = trend_with_columns.select(id_col, "IMPROVEMENT_IND_CODE", "metric_descriptor_code", "metric_descriptor_group_code", "missing_reason_code")\
                                        ...
                                        .withColumn("reporting_period_code", concat(lit("FY"), col("SUBMISSION_FISCAL_YEAR")))\
                                        ...
                                        .drop("reporting_entity_code")\
                                        .withColumnRenamed("reporting_entity_code_text", "reporting_entity_code")\
                                        .withColumn("reporting_type_code", lit("Pub"))  # Add the reporting_type_code column at the end
