...
def generate_data_for_year(year):
    ...
    for index, row in TT_Spent_ED_File.iterrows():
        ...
        if row['improvement_descriptor_code'] != '999':
            public_metric_result = '' if row['missing_reason_code'] != '999' else 'N/A'
            stacked_data.append([row['reporting_entity_code'], '', 'PerformanceTrend', row['improvement_descriptor_code'], '', row['metric_result'], public_metric_result])
        
        # For Row 3
        if row['compare_descriptor_code'] != '999':
            public_metric_result = '' if row['missing_reason_code'] != '999' else 'N/A'
            stacked_data.append([row['reporting_entity_code'], '', 'PerformanceComparison', row['compare_descriptor_code'], '', row['metric_result'], public_metric_result])
...
To implement the desired behavior where `public_metric_result` is not shown for Row 2 and Row 3 when there's a `missing_reason_code` in Row 1, you can adjust the code in the section where the data is stacked to conditionally append to the `stacked_data` list. Here's the modified code:
...
In this updated code, `public_metric_result` will not be shown for Row 2 and Row 3 when there's a `missing_reason_code` in Row 1.
