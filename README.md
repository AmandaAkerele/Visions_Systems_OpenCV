To implement the additional filtering based on the conditions you provided:

1. If the `missing_reason_code` is present, filter out the rows where `metric_descriptor_code` has a value of '999'.
2. If `missing_reason_code` is present, keep it intact in the resulting DataFrame.
...
# Assuming you have loaded your EDWT_Indicators DataFrame somewhere above this code
# Convert columns to appropriate types and apply mappings
...
def generate_data_for_year(year):
    ...
    for index, row in EDWT_Indicator_File.iterrows():
        # For Row 1
        if row['missing_reason_code']:
            if row['metric_descriptor_code'] != '999':
...
- I added an additional condition inside the loop for Row 1 to check if `missing_reason_code` is present and `metric_descriptor_code` is not '999'.
- This ensures that if `missing_reason_code` exists, the row with `metric_descriptor_code` as '999' will be filtered out, but the `missing_reason_code` will be kept in the resulting DataFrame.
