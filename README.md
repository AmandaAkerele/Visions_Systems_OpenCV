create a READ.me file from this codes below 
...
# Create empty dictionary for intermediate dataframes (will be written as intermediate job output)
    ...
    for index in range(0, 5):
        ...
        ed_nodup_nosb = ed_nodup_nosb.join(df_org_dim, 'org_id', 'left') \
        .select(
            ed_nodup_nosb["*"],
            df_org_dim['corp_id'],
        ...
        ed_nodup_nosb = ed_nodup_nosb.withColumn('corp_id', F.when(ed_nodup_nosb.corp_id.isNull(), ed_nodup_nosb.org_id).otherwise(ed_nodup_nosb.corp_id))
            
            # when corp_name is blank replace with org_name
        ed_nodup_nosb = ed_nodup_nosb.withColumn('corp_pll_name_e_desc', F.when(ed_nodup_nosb.corp_id.isNull(), ed_nodup_nosb.corp_pll_name_e_desc) \
                                           .otherwise(ed_nodup_nosb.corp_pll_name_e_desc))
            
         # Remove UCCs and Stillbirths for corp level~ ed_nodup_nosb_noucc
        ed_nodup_noucc_nosb = ed_nodup_nosb.filter(~col('FACILITY_AM_CARE_NUM').isin([row['FACILITY_AM_CARE_NUM'] for row in ucc_fac_count.select('FACILITY_AM_CARE_NUM').collect()]))
    
        
        # Group by 'CORP_ID' and count 'AM_CARE_KEY', then rename the column to 'ED_CNT'
        ED_CORP_df = ed_nodup_noucc_nosb.groupBy('CORP_ID').agg(F.count('AM_CARE_KEY').alias('ED_CNT'))
        ...
        # TPIA with UCC 
        # Create ed_record_admit_with_ucc (TPIA with UCC for National, Provincial and Regional)
        ed_record_admit_with_ucc = ed_nodup_nosb.filter(~col('VISIT_DISPOSITION').isin(['61', '63']))
        
        
        # TPIA without UCC
        ed_record_admit_noucc = ed_nodup_noucc_nosb.filter(~F.col('VISIT_DISPOSITION').isin(['61', '63']))
        # ed_record_admit_noucc = ed_records_bb_df
        
        #for corp level
        #Exclusion criteria for corp level. Exclude PS & DQ. DQ is dropped from 2024
        ed_record_corp = ed_record_admit_noucc.filter(~col('FACILITY_AM_CARE_NUM').isin([row['facility_am_care_num'] for row in 
                                                                                ps.select('facility_am_care_num').collect()]))
        
        # For the PEER group, remove UCC and standalone facilities
        # Create ed_record_Peer without filtering based on SL_filtered
        ed_record_Peer = ed_record_admit_noucc
        ...
        ed_record = ed_record_corp.withColumn(
            'tpia_rec',
            F.when(
        ...
        ed_record_admit_with_ucc = ed_record_admit_with_ucc.withColumn('tpia_rec', 
                                                                F.when(ed_record_admit_with_ucc['TIME_PHYSICAN_INIT_ASSESSMENT']
                                                                       == '9999', 'N')                                              .when(ed_record_admit_with_ucc['TIME_PHYSICAN_INIT_ASSESSMENT'].isNotNull() & 
        (ed_record_admit_with_ucc['TIME_PHYSICAN_INIT_ASSESSMENT'] != '9999'), 'Y')
                                                                  .otherwise('B'))
        
        # Filter out rows with 'tpia_rec' equal to "B"
        tpia_org_cnt_ucc = ed_record_admit_with_ucc.filter(ed_record_admit_with_ucc['tpia_rec'] != 'B')
        
        # Aggregation
        tpia_org_rec_ucc = tpia_org_cnt_ucc.groupBy('SUBMISSION_FISCAL_YEAR', 'CORP_ID').agg(
            F.count('AM_CARE_KEY').alias('Total_CASE'),
        ...
        tpia_supp_org_ucc = tpia_org_rec_ucc.filter((col('tpia_calc_cnt') < 50) | ((col('tpia_calc_cnt') > 50) & (col('tpia_rec_pct') < 0.75)))
        tpia_rpt_org_ucc = tpia_org_rec_ucc.filter((col('tpia_calc_cnt') >= 50) | ((col('tpia_calc_cnt') < 50) & (col('tpia_rec_pct') >= 0.75)))
        
         # Append the ucc df to make a list
        tpia_supp_org_all.append(tpia_supp_org_ucc)
        ...
        ## For Peer level
        # Assuming ed_record_Peer is already defined and available DataFrame
        # Update tpia_rec based on conditions in TIME_PHYSICAN_INIT_ASSESSMENT column
        tpia_peer_cnt_a_df = ed_record_Peer.withColumn(
            'tpia_rec',
            when(
        ...
        tpia_peer_rec_df = tpia_peer_cnt_df.groupBy('SUBMISSION_FISCAL_YEAR', 'acute_peer_group_code').agg(
             sum(when(col('tpia_rec') == 'Y', 1).otherwise(0)).alias('tpia_calc_cnt'),
             sum(when(col('tpia_rec').isin(['Y', 'N']), 1).otherwise(0)).alias('tpia_elig_cnt')
        ).withColumn(
            'tpia_rec_pct',
        ...
        # For Region
        # Apply conditions to create the 'tpia_rec' column
        ed_record_admit_with_ucc = ed_record_admit_with_ucc.withColumn(
            'tpia_rec',
        ...
        # Filter out records where 'tpia_rec' is not 'B'
        tpia_reg_cnt = ed_record_admit_with_ucc.filter(F.col('tpia_rec') != 'B')
        ...
        tpia_org_supp_for_natprovreg_df = tpia_supp_org_ucc.filter(col('tpia_calc_cnt') < 5)
        ...
        # Filter out the rows from ed_record_admit_with_ucc
        TPIA_nt_record_ucc = ed_record_admit_with_ucc.filter(~col('CORP_ID').isin(exclude_corp_ids))
        ...
        tpia_nt = TPIA_nt_record_ucc.groupBy('SUBMISSION_FISCAL_YEAR') \
                                  .agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        # tpia_nt.show()
        
        # Provincial 90th percentile TPIA
        tpia_prov= TPIA_nt_record_ucc.groupBy('SUBMISSION_FISCAL_YEAR','adm_prov_id', 'adm_prov_plldj_name_e_desc') \
                                  .agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        
        # Filter out rows where 'adm_prov_id' is null after aggregation
        tpia_prov = tpia_prov.filter(tpia_prov['adm_prov_id'].isNotNull())
        # tpia_prov.show()
        
        
        # Peer Group 90th  percentile TPIA (return back records for ed_record_Peer is higher above)
        tpia_peer= ed_record_Peer.groupBy('SUBMISSION_FISCAL_YEAR','acute_peer_group_code') \
                                  .agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        # Filter out rows where 'adm_prov_id' is null after aggregation
        tpia_peer = tpia_peer.filter(tpia_peer['acute_peer_group_code'].isNotNull())
        # tpia_peer.show()
        
        
        # Regional 90th percentile TPIA
        tpia_reg= ed_record_admit_with_ucc.groupBy('SUBMISSION_FISCAL_YEAR','adm_prov_plldj_name_e_desc', 'adm_region_id', 
                                                   'adm_region_pll_name_e_desc') \
                                  .agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        # Filter out rows where 'adm_prov_id' is null after aggregation
        tpia_reg = tpia_reg.filter(tpia_reg['adm_region_id'].isNotNull())
        # tpia_reg.show()
        
        # Calculate for SITE_ID/NAME/PEER
        tpia_site= ed_record.groupBy('SUBMISSION_FISCAL_YEAR','FACILITY_AM_CARE_NUM', 'acute_peer_group_code') \
                                  .agg(F.percentile_approx('WAIT_TIME_TO_PIA_HOURS', 0.9).alias('PERCENTILE_90'))
        # Filter out rows where 'adm_prov_id' is null after aggregation
        tpia_site = tpia_site.filter(tpia_site['FACILITY_AM_CARE_NUM'].isNotNull())
        # tpia_site.show()
        ...
        TPIA_site_Huron_Perth = tpia_site.filter(col('FACILITY_AM_CARE_NUM').isin([54074,54075,54102,54168]))
        ...
        tpia_corp_alias = tpia_corp.alias("tpia_corp") 
        oi_alias = df_org_dim.alias("oi") 
        # Perform the join and select 
        tpia_org = tpia_corp_alias.join(oi_alias, tpia_corp_alias['CORP_ID'] == oi_alias['org_id'], how='left')\
        .select( col("tpia_corp.CORP_ID"),col("tpia_corp.SUBMISSION_FISCAL_YEAR"),col("tpia_corp.PERCENTILE_90"),("oi.acute_peer_group_code"))
        # Show the result 
        
        
        ## **STEP 4_IND_TPIA:**
        
        #Perform the join operation on tpia_org
        ...
        tpia_org_ta = tpia_org.filter(tpia_conditions).filter((col('CORP_ID') != 80228))
        # tpia_org_ta.show() # recheck value over by 34values. need to recheck tpia_org
        tpia_org_all.append(tpia_org_ta)
        tpia_org_combined = reduce(lambda x, y: x.unionAll(y), tpia_org_all)
        
        #Perform Join operation 
        tpia_reg_ta = tpia_reg.join(
            tpia_supp_reg, 
            tpia_reg['adm_region_id'] == tpia_supp_reg['adm_region_id'],
            'left_anti'
        )
        
        tpia_reg_all.append(tpia_reg_ta)
        tpia_reg_combined = reduce(lambda x, y: x.unionAll(y), tpia_reg_all)
        
        tpia_peer_aa = tpia_peer.withColumn('ID', when(col('acute_peer_group_code') == 'T', 2)\
        ...
        tpia_nt = tpia_nt.withColumn("ORG_ID", lit(1))
        tpia_prov = tpia_prov.withColumnRenamed("adm_prov_id", "ORG_ID")
        tpia_reg = tpia_reg.withColumnRenamed("adm_region_id", "ORG_ID")
        tpia_org = tpia_org.withColumnRenamed("CORP_ID", "ORG_ID").withColumnRenamed("acute_peer_group_code", 
                                                                                           "reporting_entity_code")
        tpia_peer= tpia_peer_aa.withColumnRenamed("ID", "ORG_ID").withColumnRenamed("acute_peer_group_code", 
                                                                                       "reporting_entity_code")
        ...
        tpia_nt = tpia_nt.select([col(c).alias(c) if c in tpia_nt.columns else lit(None).alias(c) for c in 
                                        common_columns])
        tpia_prov = tpia_prov.select([col(c).alias(c) if c in tpia_prov.columns else lit(None).alias(c) for c in 
                                            common_columns])
        tpia_reg = tpia_reg.select([col(c).alias(c) if c in tpia_reg.columns else lit(None).alias(c) for c in 
                                          common_columns])
        tpia_org = tpia_org.select([col(c).alias(c) if c in tpia_org.columns else lit(None).alias(c) for c in 
                                          common_columns])
        tpia_peer = tpia_peer.select([col(c).alias(c) if c in tpia_peer.columns else lit(None).alias(c) for c in 
                                            common_columns])
        
        # Union the DataFrames
        
        stacked_df = tpia_nt.unionByName(tpia_prov) \
                           .unionByName(tpia_reg) \
                           .unionByName(tpia_org) \
                           .unionByName(tpia_peer) \
                           .orderBy('ORG_ID')
    ...
    corp_comp_alias = corp_comp.alias("tpia_org")
    ...
    result_corp = corp_comp_alias.join(tpia_peer_base_alias, on="acute_peer_group_code", how="inner").select(
        col("tpia_org.CORP_ID"),
        col("tpia_org.SUBMISSION_FISCAL_YEAR"),
        col("tpia_org.PERCENTILE_90"),
        col("tpia_org.acute_peer_group_code"),
    ...
    intermediate_data_dict["tpia_org_combined" + str(closed_year) + "_" + datetime.now().strftime('%Y_%m_%d_%H_%M_%S')] = tpia_org_combined
    intermediate_data_dict["tpia_reg_combined" + str(closed_year) + "_" + datetime.now().strftime('%Y_%m_%d_%H_%M_%S')] = tpia_reg_combined
    intermediate_data_dict["tpia_supp_reg_combined" + str(closed_year) + "_" + datetime.now().strftime('%Y_%m_%d_%H_%M_%S')] = tpia_supp_reg_combined
    intermediate_data_dict["tpia_supp_corp_combined" + str(closed_year) + "_" + datetime.now().strftime('%Y_%m_%d_%H_%M_%S')] = tpia_supp_corp_combined
    intermediate_data_dict["tpia_supp_peer_combined" + str(closed_year) + "_" + datetime.now().strftime('%Y_%m_%d_%H_%M_%S')] = tpia_supp_peer_combined
    ...
    print("done")
    return output_df_2, intermediate_data_dict
